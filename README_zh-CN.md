...
本开源项目旨在为强大但基于命令行（CLI）驱动的 OpenClaw 提供一个优雅的 GUI 替代方案。通过精致且现代的高级「暗黑模式」面板，让你可以在浏览器中轻松总览、配置监控并调度所有的自动化智能体。

## ✨ 核心特性

- **现代 & 响应式 UI**: 深度打磨的玻璃拟物设计，提供多套清晰的主题方案。
- **双主题自由切换**: 可以在“高级暗黑 (Premium Dark)”与“复古像素 (Retro Pixel)”两套风格之间无缝切换。
...
- **精细化的定时功能管理**:
  ...
  - **操作反馈感知**: 运行与删除等操作过程增加直观的加载动画（Spinner）反馈。
- **全功能可视化编辑器**:
...
- **智能体 (Sub-Agent) 可视化舰队管理**:
  - 拥有完整的图形界面用来管理独立运作的 `openclaw agents` 分身。
  - 严格遵循官方 CLI SOP 的新建流程（将自动生成工作区路径、继承主身份凭据、并静默重载网关服务配置）。
  - 支持即时编辑分身元数据（名称、专属 Emoji 头像）。
  - 内置网络自适应重试抵抗高频并发时云端防火墙产生的 SSH 断流断连。
